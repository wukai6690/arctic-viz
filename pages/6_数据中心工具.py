...
<style>
    ...
    .download-card {
        ...
        margin: 8px 0; border-left: 3px solid #0f766e;
    }
</style>
...
def render_sidebar():
    with st.sidebar:
        ...
        <div style="text-align:center; padding: 12px 0 20px 0;">
            ...
            <div style="font-size:1.05rem; font-weight:700; color:white !important; margin-top:6px;">数据中心</div>
            <div style="font-size:0.7rem; color:rgba(255,255,255,0.55);">模块 6 · v5.0</div>
        </div>
        """, unsafe_allow_html=True)
...
# -------------------------------------------------------------------------
with tab1:
    st.markdown('<div class="section-title">📥 平台数据集下载</div>', unsafe_allow_html=True)
    st.markdown("本平台提供以下可供下载的数据集，所有数据均经过清洗和聚合处理，可直接用于学术研究和二次分析。")
        ...
        with col_info:
            ...
            <div class="download-card">
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">
                    <span style="background:#ecfdf5;color:#0f766e;padding:3px 10px;border-radius:8px;font-size:0.72rem;font-weight:600;">{ds['format']}</span>
                    <span style="font-weight:700;font-size:0.95rem;">{ds['name']}</span>
                </div>
                <div style="font-size:0.82rem;color:#64748b;margin-bottom:6px;">{ds['desc']}</div>
                <div style="font-size:0.72rem;color:#94a3b8;">行数: {ds['rows']} · 大小: {ds['size']} · 来源: {ds['source']}</div>
            </div>
            """, unsafe_allow_html=True)
        with col_dl:
            # 构造正确路径
            base_dir = os.path.join(os.path.dirname(__file__), '..')
            file_path = os.path.join(base_dir, 'data', 'processed', ds['file'])
            geo_path = os.path.join(base_dir, 'geojson', ds['file'])
...
# -------------------------------------------------------------------------
with tab2:
    st.markdown('<div class="section-title">📤 上传数据可视化工具</div>', unsafe_allow_html=True)
    ...
    if uploaded:
        try:
            ...
            col_info_df = pd.DataFrame({
                ...
                '非空数': [int(df[c].notna().sum()) for c in df.columns],
            ...
            if numeric_cols:
                chart_type = st.selectbox("图表类型", ["折线图", "柱状图", "散点图", "热力图", "面积图"], key="upload_chart_type")
                viz_cols = st.columns(2)
                with viz_cols[0]:
                    x_col = st.selectbox("X轴（列）", all_cols, index=0, key="upload_x_col")
                with viz_cols[1]:
                    y_col = st.selectbox("Y轴（数值列）", numeric_cols if numeric_cols else all_cols, key="upload_y_col")
                ...
                if chart_type == "折线图":
                    ...
                    color_col = st.selectbox("颜色分组（可选）", [None] + all_cols, key="upload_color_col")
                    if color_col:
...
# -------------------------------------------------------------------------
with tab3:
    st.markdown('<div class="section-title">🔍 时空查询工具</div>', unsafe_allow_html=True)
    ...
    q_cols = st.columns(3)
    with q_cols[0]:
        query_year = st.number_input("查询年份", min_value=1979, max_value=2024, value=2020, key="q_yr_v6")
    with q_cols[1]:
        query_month = st.selectbox("查询月份", ["全部"] + list(range(1, 13)),
                                   format_func=lambda x: "全部" if x == "全部" else f"{x}月",
                                   key="q_mo_v6")
    with q_cols[2]:
        metric_type = st.selectbox("指标类型", ["年均值", "年最大", "年最小"], key="q_mt_v6")
    ...
    if query_year in ice_summary.index:
        ...
        compare_years = st.multiselect(
            "对比年份", list(range(1979, 2025)),
            default=[query_year, 1990, 2000, 2010], key="comp_yr_v6"
        )
        if compare_years:
            comp_data = []
            ...
            fig_comp = go.Figure(go.Bar(
                ...
                marker_color=['#dc2626' if yr == query_year else '#0f766e' for yr in comp_df['年份']],
    ...
    with q2_cols[0]:
        q2_year = st.number_input("查询年份", min_value=2018, max_value=2024, value=2023, key="q2y_v6")
    with q2_cols[1]:
        country_options = ["全部"] + (sorted(yc_df['CountryCode'].unique().tolist()) if not yc_df.empty else [])
        q2_country = st.selectbox("国家/地区", country_options, key="q2c_v6")
    with q2_cols[2]:
        cat_options = ["全部"] + (list(yc_df['EventCategory'].unique()) if 'EventCategory' in yc_df.columns else [])
        q2_cat = st.selectbox("事件类型", cat_options, key="q2cat_v6")
    ...
    if not yc_df.empty:
        ...
        if not q2_data.empty:
            total_events = int(q2_data['EventCount'].sum())
            avg_tone = float(q2_data['AvgTone'].mean())
            cc = st.columns(2)
            with cc[0]:
                st.metric(f"{q2_year}年事件总数", f"{total_events:,}")
            with cc[1]:
                st.metric("平均情感值", f"{avg_tone:.2f}",
...
# -------------------------------------------------------------------------
with tab4:
    st.markdown('<div class="section-title">📊 多源数据对比分析</div>', unsafe_allow_html=True)
    st.markdown("在同一图表中对比多个数据源的时间演变趋势，数据自动归一化到 0-100 范围。")

    compare_options = st.multiselect(
        "选择要对比的指标",
        ["海冰年均面积", "GDELT事件总数", "航道通航潜力", "专利申请量"],
        default=["海冰年均面积", "GDELT事件总数"],
        key="compare_options_v6"
    )
    ...
    if compare_options:
        ...
        if "海冰年均面积" in compare_options:
            ice_vals = [float(ice_summary.loc[y, 'mean']) if y in ice_summary.index else None for y in years]
        ...
        if "GDELT事件总数" in compare_options:
            ...
            compare_data['GDELT事件'] = [float(gdelt_yearly.get(y, 0)) for y in years]

        if "航道通航潜力" in compare_options:
            ice_df, _, _ = load_ice_data()
            potential = [(15 - min(float(ice_df.loc[y, 'sep']), 15)) / 10 * 100
        ...
        if "专利申请量" in compare_options:
            ...
            compare_data['专利申请'] = [float(pat_yearly.get(y, 0)) for y in years]
        ...
        colors = ['#1565C0', '#dc2626', '#16a34a', '#ea580c', '#7c3aed']
...
st.divider()
st.caption("数据说明: 本平台数据均为模拟/示例数据，仅供功能演示。真实数据请替换 data/processed/ 目录下的文件。")
