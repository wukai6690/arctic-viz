...
from src.viz import COUNTRY_NAMES, COUNTRY_COLORS, CATEGORY_COLORS, CAT_LABELS, create_network_graph, create_word_freq_chart
...
net_data = load_geopolitics_network()
...
with tab1:
    ...
    with col_map:
            ...
            for feature in stations_data.get('features', []):
                ...
                <div style="min-width:250px">
                    ...
                    <table style="width:100%;font-size:12px">
                        ...
                        <tr><td colspan="2"><b>简介：</b>{props.get('description', '')[:100]}</td></tr>
                    </table>
                </div>
                """
        ...
        if show_events and not grid_df.empty:
            year_data = grid_df[grid_df.get('Year_local', grid_df.get('Year', 2023)) == year_select]
            if not year_data.empty:
                heat_data = [
                    [row.get('lat_grid', row.get('lat', 0)),
                     row.get('lon_grid', row.get('lon', 0)),
                     row.get('EventCount', 1)]
        ...
        # 添加航道
        if show_routes:
            routes_data = json.load(open(os.path.join(os.path.dirname(__file__), '..', 'geojson', 'arctic_routes.geojson'), encoding='utf-8'))
            ...
            for feature in routes_data.get('features', []):
                ...
                folium.PolyLine(
                    ...
                    popup=f"<b>{name}</b><br>{props.get('description', '')}"
                ).add_to(route_group)
...
with tab2:
    ...
    with rel_cols[0]:
        ...
        <div style="display:flex;align-items:center;gap:8px;margin:4px 0">
            <div style="width:30px;height:3px;background:#43A047;border-radius:2px;"></div>
            <span>合作关系</span>
        </div>
        """, unsafe_allow_html=True)
    with rel_cols[1]:
        st.markdown("""
        <div style="display:flex;align-items:center;gap:8px;margin:4px 0">
            <div style="width:30px;height:3px;background:#FF6B35;border-radius:2px;border-style:dashed;"></div>
            <span>竞争关系</span>
        </div>
        """, unsafe_allow_html=True)
    with rel_cols[2]:
        st.markdown("""
        <div style="display:flex;align-items:center;gap:8px;margin:4px 0">
            <div style="width:30px;height:3px;background:#E53935;border-radius:2px;border-style:dotted;"></div>
            <span>对抗关系</span>
        </div>
        """, unsafe_allow_html=True)

    fig_net = create_network_graph(net_data)
    ...
    with stat_cols[3]:
        conflict_count = sum(1 for l in net_data['links'] if l['relation'] == 'confrontation')
        st.metric("对抗关系", conflict_count)
    ...
    st.dataframe(
        rel_df[['source_name', 'relation_cn', 'target_name', 'strength', 'period']].rename(
            columns={'source_name': '主体A', 'relation_cn': '关系', 'target_name': '主体B', 'strength': '强度', 'period': '时期'}
        ),
...
with tab3:
    ...
    st.caption("基于各国北极政策白皮书文本，提取高频关键词")

    # 情感分析柱状图
    import plotly.graph_objects as go

    countries_sorted = sorted(policy_texts.keys(), key=lambda x: policy_texts[x]['sentiment'])
    sentiments = [policy_texts[c]['sentiment'] for c in countries_sorted]
    colors = ['#43A047' if s > 0 else '#E53935' for s in sentiments]

    fig_sent = go.Figure(go.Bar(
        x=sentiments, y=[policy_texts[c]['name'] for c in countries_sorted],
        orientation='h',
        marker_color=colors,
        hovertemplate='%{y}: %{x:.2f}<extra></extra>'
    ))
    fig_sent.add_vline(x=0, line_dash='dash', line_color='gray')
    fig_sent.update_layout(
        xaxis_title='情感值 (-10负面 ~ +10正面)',
        yaxis_title='国家',
        height=300,
    ...
    **情感分析解读：** 中国的北极政策文本情感值最高，反映合作导向的治理观；
    俄罗斯近年情感值转负，体现军事对抗性增强；北欧国家普遍呈正面倾向，
    强调环境保护与国际合作。
    """)

    # 关键词柱状图
    st.markdown("#### 政策关键词词频对比")
    fig_wc = create_word_freq_chart(policy_texts)
    ...
    country_data = {
        ...
        'NOR': ('挪威', '#FF6B35', '推动巴伦支海合作与环境保护，主张以国际法框架解决争端'),
    }
...
with tab4:
    ...
    if not features:
        ...
        for feature in features:
            ...
            lat, lon = geom.get('coordinates', [0, 0])[1], geom.get('coordinates', [0, 0])[0]

            color_map = {
                '中国': '#E53935', '美国': '#1565C0', '俄罗斯': '#8B0000',
                '挪威': '#FF6B35', '丹麦': '#FFA726', '芬兰': '#9C27B0',
                '瑞典': '#00BCD4', '国际合作（多国）': '#607D8B',
                '丹麦/美国': '#FFA726', '挪威/国际': '#FF6B35'
            }
